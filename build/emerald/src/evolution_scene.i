...
{
    u32 data = 0;
    if (gEvolutionTable[preEvoSpecies][0].method == 0x000d && gPlayerPartyCount < 6)
    {
        s32 i;
...
static void Task_EvolutionScene(u8 taskID)
{
    ...
    case 2:
        ...
        {
            EvoScene_DoMonAnimation(sEvoStructPtr->preEvoSpriteID, gTasks[taskID].data[1]);
            gTasks[taskID].data[0]++;
        }
        break;
    case 3:
    ...
    case 13:
        if (!gPaletteFade.active)
        {
            EvoScene_DoMonAnimation(sEvoStructPtr->postEvoSpriteID, gTasks[taskID].data[2]);
            gTasks[taskID].data[0]++;
        }
        break;
    case 14:
...
static void Task_TradeEvolutionScene(u8 taskID)
{
    ...
    case 1:
        if (!IsTextPrinterActive(0))
        {
            PlayCry1(gTasks[taskID].data[1], 0);
            gTasks[taskID].data[0]++;
        }
        break;
    case 2:
