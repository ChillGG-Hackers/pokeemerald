...
static void PlayerBufferRunCommand(void)
{
    ...
    {
        if (gBattleBufferA[gActiveBattler][0] < (size_t)(sizeof(sPlayerBufferCommands) / sizeof((sPlayerBufferCommands)[0])))
...
static void HandleInputChooseTarget(void)
{
    s32 i;
    u8 identities[4];
    memcpy(identities, sTargetIdentities, (size_t)(sizeof(sTargetIdentities) / sizeof((sTargetIdentities)[0])));
        ...
        do
        {
            ...
            case 2:
                if (gActiveBattler != gMultiUsePlayerCursor)
                    i++;
                else if (gBattleMoves[GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], 13 + gMoveSelectionCursor[gActiveBattler])].target & 0x2)
                    i++;
                break;
            case 1:
            case 3:
...
static void HandleInputChooseMove(void)
{
    ...
    if (gMain.newKeys & 0x0001)
    {
        ...
        {
            if (moveTarget & 0x2 && !gBattleBufferA[gActiveBattler][2])
                canSelectTarget++;
        }
        else
        {
            if (!(moveTarget & (0x4 | 0x8 | 0x1 | 0x20 | 0x40 | 0x10)))
                canSelectTarget++;
    ...
    {
        if (gNumberOfMovesToChoose > 1 && !(gBattleTypeFlags & 0x0002))
        {
...
static void HandleMoveSwitching(void)
{
    ...
    if (gMain.newKeys & (0x0001 | 0x0004))
    {
        ...
        if (gMoveSelectionCursor[gActiveBattler] != gMultiUsePlayerCursor)
        {
            ...
            if (!(gBattleMons[gActiveBattler].status2 & 0x00200000))
            {
                for (i = 0; i < 4; i++)
                {
...
static void sub_805896C(void)
{
    ...
    if (var && gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].field_1_x1
        && gBattleSpritesDataPtr->healthBoxesData[gActiveBattler ^ 2].field_1_x1)
    {
...
static void sub_8058B40(void)
{
    ...
    if (!gBattleSpritesDataPtr->healthBoxesData[gActiveBattler ^ 2].flag_x80 && !gBattleSpritesDataPtr->healthBoxesData[gActiveBattler ^ 2].ballAnimActive)
        sub_8172EF0(gActiveBattler ^ 2, &gPlayerParty[gBattlerPartyIndexes[gActiveBattler ^ 2]]);
...
static void PlayerHandleSuccessBallThrowAnim(void)
{
    ...
    gDoingBattleAnim = 1;
    InitAndLaunchSpecialAnimation(gActiveBattler, gActiveBattler, GetBattlerAtPosition(1), 0x3);
...
static void PlayerHandleChoosePokemon(void)
{
    ...
    if (gBattleTypeFlags & 0x40000 && (gBattleBufferA[gActiveBattler][1] & 0xF) != 2)
    {
