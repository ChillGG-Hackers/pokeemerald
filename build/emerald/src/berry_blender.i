...
struct BerryBlenderData
{
    ...
    s16 field_4C;
    u16 max_RPM;
    u8 syncArrowSpriteIds[4];
    u8 syncArrowSprite2Ids[4];
    u8 unused_57[0xB];
    u8 gameEndState;
    u16 field_64[4];
    u16 field_6C;
...
__attribute__((section("ewram_data"))) static struct BerryBlenderData *sBerryBlenderData = ((void *)0);
__attribute__((section("ewram_data"))) static s32 sUnknown_020322A8[5] = {0};
__attribute__((section("ewram_data"))) static s32 sUnknown_020322BC[5] = {0};
__attribute__((section("ewram_data"))) static u32 sUnknown_020322D0 = 0;
...
static const u8 sBlenderSyncArrowsPos[4][2] =
{
...
static void sub_807FAC8(void)
{
    ...
    case 0:
        ...
        InitBgsFromTemplates(1, sBerryBlenderBgTemplates, (size_t)(sizeof(sBerryBlenderBgTemplates) / sizeof((sBerryBlenderBgTemplates)[0])));
...
{
    const struct Berry *berryInfo = GetBerryInfo((((itemId - 133) + 1)));
...
static void Blender_SetOpponentsBerryData(u16 playerBerryItemId, u8 playersNum, struct BlenderBerry* playerBerry)
{
    ...
    {
        opponentSetId = playerBerryItemId - 133;
        if (opponentSetId >= 5)
    ...
    for (i = 0; i < playersNum - 1; i++)
    {
        ...
        Blender_SetParticipantBerryData(i + 1, opponentBerryId + 133);
    }
}
    ...
    for (i = 0; i < sBerryBlenderData->playersNo; i++)
    {
        if (sub_80814B0(gRecvCmds[i][0], 0x4444, 0x4400))
        {
            ...
            else if (gRecvCmds[i][2] == 0x5432)
            {
                ...
                sBerryBlenderData->scores[i][1]++;
            }
            else if (gRecvCmds[i][2] == 0x2345)
            {
                sub_8081288(0x2345, var);
                sub_8081370(0x2345);
                if (sBerryBlenderData->scores[i][2] < 999)
                    sBerryBlenderData->scores[i][2]++;
            }
            if (gRecvCmds[i][2] == 0x2345 || gRecvCmds[2][i] == 0x4523 || gRecvCmds[2][i] == 0x5432)
            {
                if (sBerryBlenderData->field_4C > 1500)
...
static void sub_8081898(void)
{
    ...
    if (sBerryBlenderData->gameFrameTime < (99 * 60 * 60) + (59 * 60))
...
static bool8 sub_8081964(struct BlenderBerry* berries, u8 index1, u8 index2)
{
    if (berries[index1].itemId != berries[index2].itemId
     || (StringCompare(berries[index1].name, berries[index2].name) == 0
      && (berries[index1].flavors[0] == berries[index2].flavors[0]
       && berries[index1].flavors[1] == berries[index2].flavors[1]
       && berries[index1].flavors[2] == berries[index2].flavors[2]
       && berries[index1].flavors[3] == berries[index2].flavors[3]
       && berries[index1].flavors[4] == berries[index2].flavors[4]
    ...
    for (i = 0; i < playersNo; i++)
    {
        for (r6 = 0; r6 < playersNo; r6++)
        {
            if (berries[i].itemId == berries[r6].itemId && i != r6
                && (berries[i].itemId != 175 || sub_8081964(berries, i, r6)))
                    return 12;
        }
    }
    r2 = 0;
    ...
    case 10:
        if (!gPaletteFade.active)
        {
            if (sBerryBlenderData->field_64[0] == 0x2222)
            {
                ...
                UnsetBgTilemapBuffer(2);
                UnsetBgTilemapBuffer(1);
                { Free(sBerryBlenderData); sBerryBlenderData = ((void *)0); };
...
static void BerryBlender_SetBackgroundsPos(void)
{
    SetGpuReg(0x14, sBerryBlenderData->bg_X);
    SetGpuReg(0x16, sBerryBlenderData->bg_Y);

    SetGpuReg(0x10, sBerryBlenderData->bg_X);
    SetGpuReg(0x12, sBerryBlenderData->bg_Y);
}
...
static void sub_8083230(u16 a0)
{
    ...
    *((u16*)(0x6000000 + 0x6458)) = palAdders[4] + 0x8072;
    *((u16*)(0x6000000 + 0x645A)) = palAdders[3] + 0x8072;
    *((u16*)(0x6000000 + 0x645C)) = palAdders[2] + 0x8072;
    *((u16*)(0x6000000 + 0x6460)) = palAdders[1] + 0x8072;
    *((u16*)(0x6000000 + 0x6462)) = palAdders[0] + 0x8072;
}
    ...
    case 3:
        {
            ...
            for (i = 0; i < sBerryBlenderData->playersNo; yPos += 16, i++)
            {
                ...
                Blender_AddTextPrinter(5, sBerryBlenderData->stringVar, 8, yPos, 0xFF, 3);
                ...
                Blender_AddTextPrinter(5, sBerryBlenderData->stringVar, 0x54, yPos, 0xFF, 3);
            }
            ...
            Blender_AddTextPrinter(5, sBerryBlenderData->stringVar, xPos, 0x51, 0xFF, 3);
            ...
            xPos = GetStringRightAlignXOffset(1, sBerryBlenderData->stringVar, 0xA8);
            Blender_AddTextPrinter(5, sBerryBlenderData->stringVar, xPos, 0x61, 0xFF, 3);
...
static void Blender_SortScores(void)
{
    ...
    for (i = 0; i < sBerryBlenderData->playersNo; i++)
    {
        points[i] = 1000000 * sBerryBlenderData->scores[i][0];
        points[i] += 1000 * sBerryBlenderData->scores[i][1];
        points[i] += 1000 - sBerryBlenderData->scores[i][2];
    }
...
static bool8 Blender_PrintBlendingRanking(void)
{
    ...
    case 3:
        ...
        sBerryBlenderData->scoreIconIds[0] = CreateSprite(&sUnknown_08339B40, 128, 52, 0);
        StartSpriteAnim(&gSprites[sBerryBlenderData->scoreIconIds[0]], 3);
        gSprites[sBerryBlenderData->scoreIconIds[0]].callback = SpriteCallbackDummy;

        sBerryBlenderData->scoreIconIds[1] = CreateSprite(&sUnknown_08339B40, 160, 52, 0);
        gSprites[sBerryBlenderData->scoreIconIds[1]].callback = SpriteCallbackDummy;

        sBerryBlenderData->scoreIconIds[2] = CreateSprite(&sUnknown_08339B40, 192, 52, 0);
        StartSpriteAnim(&gSprites[sBerryBlenderData->scoreIconIds[2]], 1);
        gSprites[sBerryBlenderData->scoreIconIds[2]].callback = SpriteCallbackDummy;
        ...
        for (yPos = 0x29, i = 0; i < sBerryBlenderData->playersNo; yPos += 0x10, i++)
        {
            ...
            StringAppend(sBerryBlenderData->stringVar, gText_Space);
            StringAppend(sBerryBlenderData->stringVar, gLinkPlayers[place].name);
            Blender_AddTextPrinter(5, sBerryBlenderData->stringVar, 0, yPos, 0xFF, 3);

            ConvertIntToDecimalStringN(sBerryBlenderData->stringVar, sBerryBlenderData->scores[place][0], STR_CONV_MODE_RIGHT_ALIGN, 3);
            Blender_AddTextPrinter(5, sBerryBlenderData->stringVar, 0x4E, yPos, 0xFF, 3);

            ConvertIntToDecimalStringN(sBerryBlenderData->stringVar, sBerryBlenderData->scores[place][1], STR_CONV_MODE_RIGHT_ALIGN, 3);
            Blender_AddTextPrinter(5, sBerryBlenderData->stringVar, 0x6E, yPos, 0xFF, 3);

            ConvertIntToDecimalStringN(sBerryBlenderData->stringVar, sBerryBlenderData->scores[place][2], STR_CONV_MODE_RIGHT_ALIGN, 3);
            Blender_AddTextPrinter(5, sBerryBlenderData->stringVar, 0x8E, yPos, 0xFF, 3);
        }
...
void ShowBerryBlenderRecordWindow(void)
{
    ...
    AddTextPrinterParameterized(gRecordsWindowId, 1, gText_234Players, 4, 0x29, 0, ((void *)0));

    for (i = 0, yPos = 0x29; i < 3; i++)
    {
        u8 *txtPtr;
        u32 record;
