...
static __attribute__((section("ewram_data"))) u8 sUnknown_0203CF50 = 0;
...
static u8 sub_81D4890(u8 ignoredDir)
{
    ...
    if (CanMewWalkToCoords(mew->currentCoords.x, mew->currentCoords.y - 1) == 1 && ignoredDir != 2)
    {
        sMewDirectionCandidates[count] = 2;
        count++;
    }

    if (CanMewWalkToCoords(mew->currentCoords.x + 1, mew->currentCoords.y) == 1 && ignoredDir != 4)
    {
        sMewDirectionCandidates[count] = 4;
        count++;
    }

    if (CanMewWalkToCoords(mew->currentCoords.x, mew->currentCoords.y + 1) == 1 && ignoredDir != 1)
    {
        sMewDirectionCandidates[count] = 1;
        count++;
    }

    if (CanMewWalkToCoords(mew->currentCoords.x - 1, mew->currentCoords.y) == 1 && ignoredDir != 3)
    {
...
{
    return sMewDirectionCandidates[VarGet(0x403A) % mod];
}
