...
static bool8 ShouldSwitchIfWonderGuard(void)
{
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        if (GetMonData(&party[i], 65) == 0)
            continue;
        if (GetMonData(&party[i], 65) == 412)
            continue;
        if (i == gBattlerPartyIndexes[gActiveBattler])
            continue;

        GetMonData(&party[i], 11);
        GetMonData(&party[i], 46);
...
static bool8 ShouldSwitch(void)
{
    ...
    if (gBattleMons[*(activeBattlerPtr = &gActiveBattler)].status2 & (0x0000E000 | 0x04000000))
        return 0;
    if (gStatuses3[gActiveBattler] & 0x400)
        return 0;
    if ((AbilityBattleEffects(0xC, gActiveBattler, 23, 0, 0)))
        return 0;
    if ((AbilityBattleEffects(0xC, gActiveBattler, 71, 0, 0)))
        return 0;
    if ((AbilityBattleEffects(0xE, 0, 42, 0, 0)))
    {
        if (gBattleMons[gActiveBattler].type1 == 8)
            return 0;
        if (gBattleMons[gActiveBattler].type2 == 8)
            return 0;
    }
    ...
    if (gBattleTypeFlags & 0x0001)
    {
        ...
        if (gAbsentBattlerFlags & gBitTable[GetBattlerAtPosition(GetBattlerPosition(*activeBattlerPtr) ^ 2)])
...
u8 GetMostSuitableMonToSwitchInto(void)
{
    ...
    while (invalidMons != 0x3F)
    {
        ...
        if (bestMonId != 6)
        {
            for (i = 0; i < 4; i++)
            {
                move = GetMonData(&party[bestMonId], 13 + i);
    ...
    for (i = firstId; i < lastId; i++)
    {
        if ((u16)(GetMonData(&party[i], 11)) == 0)
            continue;
        if (GetMonData(&party[i], 57) == 0)
            continue;
...
static bool8 ShouldUseItem(void)
{
    ...
    if (gBattleTypeFlags & 0x400000 && GetBattlerPosition(gActiveBattler) == 2)
        return 0;
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (item == 0)
            continue;
        if (gItemEffectTable[item - 13] == ((void *)0))
            continue;
        ...
        *(gBattleStruct->AI_itemType + gActiveBattler / 2) = GetAI_ItemType(item, itemEffects);
        ...
        case AI_ITEM_CURE_CONDITION:
            *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) = 0;
            if (itemEffects[3] & 0x20 && gBattleMons[gActiveBattler].status1 & 0x7)
            {
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x20;
                shouldUse = 1;
            }
            if (itemEffects[3] & 0x10 && (gBattleMons[gActiveBattler].status1 & 0x8 || gBattleMons[gActiveBattler].status1 & 0x80))
            {
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x10;
                shouldUse = 1;
            }
            if (itemEffects[3] & 0x8 && gBattleMons[gActiveBattler].status1 & 0x10)
            {
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x8;
                shouldUse = 1;
            }
            if (itemEffects[3] & 0x4 && gBattleMons[gActiveBattler].status1 & 0x20)
            {
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x4;
                shouldUse = 1;
            }
            if (itemEffects[3] & 0x2 && gBattleMons[gActiveBattler].status1 & 0x40)
            {
                *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) |= 0x2;
                shouldUse = 1;
            }
            if (itemEffects[3] & 0x1 && gBattleMons[gActiveBattler].status2 & 0x00000007)
            {
