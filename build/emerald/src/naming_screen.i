...
__attribute__((section("ewram_data"))) static struct NamingScreenData *gNamingScreenData = ((void *)0);
extern u16 gKeyRepeatStartDelay;
...
static void NamingScreen_Init(void)
{
    ...
    gNamingScreenData->bg1Priority = (1);
    gNamingScreenData->bg2Priority = (2);
...
static bool8 MainState_WaitFadeOutAndExit(void)
{
    if (!gPaletteFade.active)
    {
        ...
        { Free(gNamingScreenData); gNamingScreenData = ((void *)0); };
    }
    return 0;
}
...
static bool8 PageSwapAnimState_1(struct Task *task)
{
    ...
    task->data[1] += 4;
    *arr[gNamingScreenData->bgToReveal] = Sin(task->data[1], 40);
    *arr[gNamingScreenData->bgToHide] = Sin((task->data[1] + 128) & 0xFF, 40);
    if (task->data[1] >= 64)
    {
        u8 temp = gNamingScreenData->bg1Priority;
...
static void NamingScreen_CreateWandaDadIcon(void)
{
    ...
    spriteId = AddPseudoObjectEvent(19, SpriteCallbackDummy, 0x38, 0x25, 0);
    gSprites[spriteId].oam.priority = 3;
    StartSpriteAnim(&gSprites[spriteId], 4);
}
...
{
    FillWindowPixelBuffer(gNamingScreenData->windows[3], ((1) | ((1) << 4)));
...
static void sub_80E49BC(void)
{
    ...
    if (gNamingScreenData->monGender != 0xFF)
    {
        if (gNamingScreenData->monGender == 0xFE)
        {
...
static u8 GetTextCaretPosition(void)
{
    ...
    {
        if (gNamingScreenData->textBuffer[i] == 0xFF)
            return i;
    }
...
static void DeleteTextCharacter(void)
{
    ...
    sub_80E4D10();
    CopyBgTilemapBufferToVram(3);
    gNamingScreenData->textBuffer[index] = 0xFF;
...
static void sub_80E4BE4(void)
{
    ...
    for (i = 0; i < gNamingScreenData->template->maxChars; i++)
    {
        if (gNamingScreenData->textBuffer[i] != 0x00 && gNamingScreenData->textBuffer[i] != 0xFF)
        {
...
static void sub_80E4D10(void)
{
    ...
    FillWindowPixelBuffer(gNamingScreenData->windows[2], ((1) | ((1) << 4)));

    for (i = 0; i < maxChars; i++)
    {
        temp[0] = gNamingScreenData->textBuffer[i];
        temp[1] = gText_ExpandedPlaceholder_Empty[0];
        unk2 = (IsLetter(temp[0]) == 1) ? 2 : 0;

        AddTextPrinterParameterized(gNamingScreenData->windows[2], 1, temp, i * 8 + unk + unk2, 1, 0xFF, ((void *)0));
    }
...
{
    [KBPAGE_LETTERS_LOWER] = ((0xE) | ((0xE) << 4)),
    [KBPAGE_LETTERS_UPPER] = ((0xD) | ((0xD) << 4)),
    [KBPAGE_SYMBOLS] = ((0xF) | ((0xF) << 4))
};
...
static void sub_80E4EF0(void)
{
    ...
    FillWindowPixelBuffer(gNamingScreenData->windows[4], ((15) | ((15) << 4)));
...
static void VBlankCB_NamingScreen(void)
{
    ...
    TransferPlttBuffer();
    SetGpuReg(0x16, gNamingScreenData->bg1vOffset);
    SetGpuReg(0x1a, gNamingScreenData->bg2vOffset);
    SetGpuReg(0xa, GetGpuReg(0xa) & 0xFFFC);
    SetGpuRegBits(0xa, gNamingScreenData->bg1Priority);
    SetGpuReg(0xc, GetGpuReg(0xc) & 0xFFFC);
    SetGpuRegBits(0xc, gNamingScreenData->bg2Priority);
}
...
static const struct SubspriteTable gUnknown_0858C050[] =
{
    {(size_t)(sizeof(gUnknown_0858C004) / sizeof((gUnknown_0858C004)[0])), gUnknown_0858C004}
};

static const struct SubspriteTable gUnknown_0858C058[] =
{
    {(size_t)(sizeof(gUnknown_0858C024) / sizeof((gUnknown_0858C024)[0])), gUnknown_0858C024},
    {(size_t)(sizeof(gUnknown_0858C024) / sizeof((gUnknown_0858C024)[0])), gUnknown_0858C024},
    {(size_t)(sizeof(gUnknown_0858C024) / sizeof((gUnknown_0858C024)[0])), gUnknown_0858C024}
};
...
static const struct SubspriteTable gUnknown_0858C070[] =
{
    {(size_t)(sizeof(gUnknown_0858C02C) / sizeof((gUnknown_0858C02C)[0])), gUnknown_0858C02C}
};

static const struct SubspriteTable gUnknown_0858C078[] =
{
    {(size_t)(sizeof(gUnknown_0858C044) / sizeof((gUnknown_0858C044)[0])), gUnknown_0858C044}
};
