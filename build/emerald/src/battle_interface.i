...
{
    {(size_t)(sizeof(sUnknown_0832C220) / sizeof((sUnknown_0832C220)[0])), sUnknown_0832C220},
    {(size_t)(sizeof(sUnknown_0832C248) / sizeof((sUnknown_0832C248)[0])), sUnknown_0832C248},
    {(size_t)(sizeof(sUnknown_0832C234) / sizeof((sUnknown_0832C234)[0])), sUnknown_0832C234},
    {(size_t)(sizeof(sUnknown_0832C250) / sizeof((sUnknown_0832C250)[0])), sUnknown_0832C250}
};
    ...
    if (GetBattlerSide(battlerId) == 0)
    {
        ...
        else
        {
            for (i = 0, var = 5, j = 0; j < 6; j++)
            {
                ...
                else if (gBattleTypeFlags & 0x40000 && gBattleStruct->arenaLostPlayerMons & gBitTable[j])
                {
                    gSprites[ballIconSpritesIds[i]].oam.tileNum += 3;
                }
    ...
    else
    {
        ...
        else
        {
            for (var = 0, i = 0, j = 0; j < 6; j++)
            {
                ...
                else if (gBattleTypeFlags & 0x40000 && gBattleStruct->arenaLostOpponentMons & gBitTable[j])
                {
                    gSprites[ballIconSpritesIds[5 - var]].oam.tileNum += 3;
                }
                else if (partyInfo[j].status != 0)
...
static void sub_8074F88(struct TestingBar *barInfo, s32 *currValue, u16 *arg2)
{
    ...
    CalcBarFilledPixels(barInfo->maxValue, barInfo->oldValue,
                barInfo->receivedValue, currValue, sp8, 48 / 8);

    for (i = 0; i < 6; i++)
        sp10[i] = (barInfo->unkC_0 << 12) | (barInfo->unk10 + sp8[i]);

    CpuSet(sp10, arg2, 0x00000000 | ((sizeof(sp10))/(16/8) & 0x1FFFFF));
}
