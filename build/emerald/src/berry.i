...
const struct Berry *GetBerryInfo(u8 berry)
{
    if (berry == (((175 - 133) + 1)) && IsEnigmaBerryValid())
        return (struct Berry*)(&gSaveBlock1Ptr->enigmaBerry.berry);
    else
    {
        if (berry == 0 || berry > (((175 - 133) + 1)))
            berry = (((133 - 133) + 1));
...
bool8 IsPlayerFacingEmptyBerryTreePatch(void)
{
    if (GetObjectEventScriptPointerPlayerFacing() == BerryTreeScript
     && GetStageByBerryTreeId(ObjectEventGetBerryTreeId(gSelectedObjectEvent)) == 0)
        return 1;
    else
        return 0;
}
...
void ObjectEventInteractionPlantBerryTree(void)
{
    ...
    PlantBerryTree(ObjectEventGetBerryTreeId(gSelectedObjectEvent), berry, 1, 1);
