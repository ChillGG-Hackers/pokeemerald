...
{
    if (!InBattlePyramid())
        GoToBagMenu(1, 5, CB2_SetUpReshowBattleScreenAfterMenu2);
    else
...
void ApprenticeOpenBagMenu(void)
{
    GoToBagMenu(9, 5, CB2_ApprenticeExitBagMenu);
...
void FavorLadyOpenBagMenu(void)
{
    GoToBagMenu(7, 5, CB2_FavorLadyExitBagMenu);
    gSpecialVar_Result = 0;
}

void QuizLadyOpenBagMenu(void)
{
    GoToBagMenu(8, 5, CB2_QuizLadyExitBagMenu);
    gSpecialVar_Result = 0;
}
...
void GoToBagMenu(u8 bagMenuType, u8 pocketId, void ( *postExitMenuMainCallback2)())
{
    ...
    else
    {
        ...
        temp = gBagPositionStruct.location - (5 - 1);
        if (temp <= 1)
            gBagMenu->unk81B = 1;
...
void CB2_Bag(void)
{
    while(sub_81221EC() != 1 && SetupBagMenu() != 1 && sub_81221AC() != 1) {};
}
...
void GetItemName(s8 *dest, u16 itemId)
{
    switch (gBagPositionStruct.pocket)
    {
        case 2:
            ...
            if (itemId >= 339)
            {
                ConvertIntToDecimalStringN(gStringVar1, itemId - 339 + 1, STR_CONV_MODE_LEADING_ZEROS, 1);
                StringExpandPlaceholders(dest, gText_ClearTo11Var1Clear5Var2);
            }
            else
            {
                ConvertIntToDecimalStringN(gStringVar1, itemId - 289 + 1, STR_CONV_MODE_LEADING_ZEROS, 2);
                StringExpandPlaceholders(dest, gText_NumberVar1Clear7Var2);
            }
            break;
        case 3:
            ConvertIntToDecimalStringN(gStringVar1, itemId - 133 + 1, STR_CONV_MODE_LEADING_ZEROS, 2);
...
void PrintItemQuantityPlusGFX(u8 rboxId, s32 item_index_in_pocket, u8 a)
{
    ...
    if (item_index_in_pocket != -2)
    {
        ...
        else if (gBagPositionStruct.pocket != 4 && (unique = ItemId_GetImportance(itemId)) == 0)
        {
...
{
    if (b == 0xFF)
        FillWindowPixelRect(0, ((0) | ((0) << 4)), 0, a, GetMenuCursorDimensionByFont(1, 0), GetMenuCursorDimensionByFont(1, 1));
    else
...
{
    u8 numDigits = (gBagPositionStruct.pocket == 3) ? 3 : 2;
...
void Task_BagMenu(u8 taskId)
{
    ...
    if (sub_81221EC() != 1 && !gPaletteFade.active)
    {
        ...
        if ((select = (gMain.newKeys & 0x0004)))
        {
            if (sub_81AC2C0() == 1)
            {
...
{
    if (deltaBagPocketId == 1 && *bagPocketId == 5 - 1)
        *bagPocketId = 0;
...
void SwitchBagPocket(u8 taskId, s16 deltaBagPocketId, u16 a3)
{
    ...
    if (a3 == 0)
    {
        ...
        DestroyListMenuTask(data[0], &gBagPositionStruct.scrollPosition[gBagPositionStruct.pocket], &gBagPositionStruct.cursorPosition[gBagPositionStruct.pocket]);
        schedule_bg_copy_tilemap_to_vram(0);
        gSprites[gBagMenu->spriteId[2 + (gBagMenu->unk81B_1 ^ 1)]].invisible = 1;
        sub_81AB824();
    }
...
void sub_81AC644(u8 unused)
{
    switch (gBagPositionStruct.location - 1)
    {
        ...
        case 4:
            ...
            break;
        case 8:
            if (!ItemId_GetImportance(gSpecialVar_ItemId) && gSpecialVar_ItemId != 175)
            {
        ...
        default:
            ...
            {
                if (gBagPositionStruct.pocket == 4 || !sub_8122148(gSpecialVar_ItemId))
                {
                    gBagMenu->unk820 = &gUnknown_08614046;
                    gBagMenu->unk828 = 1;
                }
                ...
                switch (gBagPositionStruct.pocket)
                {
                    ...
                    case 4:
                        ...
                        if (gSaveBlock1Ptr->registeredItem == gSpecialVar_ItemId)
                            gBagMenu->unk825 = 8;
                        if (gSpecialVar_ItemId == 259 || gSpecialVar_ItemId == 272)
                        {
    ...
    if (gBagPositionStruct.pocket == 2)
    {
        ClearWindowTilemap(1);
...
void item_menu_type_2(u8 taskId)
{
    ...
    else if (gBagPositionStruct.pocket != 4 && !ItemId_GetImportance(gSpecialVar_ItemId))
    {
        Task_FadeAndCloseBagMenu(taskId);
    }
    else
...
void DoWallyTutorialBagMenu(void)
{
    ...
    AddBagItem(13, 1);
    AddBagItem(4, 1);
    GoToBagMenu(10, 0, CB2_SetUpReshowBattleScreenAfterMenu2);
}
