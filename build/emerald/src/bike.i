...
bool8 IsBikingDisallowedByPlayer(void)
{
    ...
    if (!(gPlayerAvatar.flags & ((1 << PLAYER_AVATAR_STATE_SURFING) | (1 << PLAYER_AVATAR_STATE_UNDERWATER))))
    {
...
bool8 player_should_look_direction_be_enforced_upon_movement(void)
{
    if (TestPlayerAvatarFlags((1 << PLAYER_AVATAR_STATE_ACRO_BIKE)) != 0 && MetatileBehavior_IsBumpySlope(gObjectEvents[gPlayerAvatar.objectEventId].currentMetatileBehavior) != 0)
        return 0;
    else
        return 1;
}
...
s16 GetPlayerSpeed(void)
{
    ...
    if (gPlayerAvatar.flags & (1 << PLAYER_AVATAR_STATE_MACH_BIKE))
        ...
        return SPEED_FASTER;
    else if (gPlayerAvatar.flags & ((1 << PLAYER_AVATAR_STATE_SURFING) | (1 << PLAYER_AVATAR_STATE_WATERING)))
        return SPEED_FAST;
    else
        return SPEED_NORMAL;
}
