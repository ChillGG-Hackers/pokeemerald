...
static __attribute__((section("ewram_data"))) u8 *sMessageBoxAllocBuffer = ((void *)0);





static __attribute__((section("ewram_data"))) u8 *sMessageBoxTileBuffers[14] = {((void *)0)};

__attribute__((section("ewram_data"))) struct MailStruct gTradeMail[6] = {0};
...
static __attribute__((section("ewram_data"))) struct {
               ...
               u8 drawPartyState[2];
               u8 selectedMonIdx[2];
               u8 playerLinkFlagChoseAction;
               u8 partnerLinkFlagChoseAction;
               u8 playerLinkFlagStatus;
               u8 partnerLinkFlagStatus;
...
} *sTradeMenuData = {((void *)0)};
static __attribute__((section("ewram_data"))) struct {
             struct Pokemon mon;
             u32 timer;
             u32 monPersonalities[2];
             u8 filler_70[2];
             u8 playerLinkFlagFinishTrade;
             u8 partnerLinkFlagFinishTrade;
             u16 linkData[10];
             u8 alwaysZero_88;
             u8 alwaysZero_89;
             u16 linkTimeoutCounter;
             u16 neverRead_8C;
             u8 pokePicSpriteIdxs[2];
             u8 unk_90;
             u8 unk_91;
             u8 unk_92;
             u8 sendTradeFinishState;
             u16 state;
...
# 176 "src/data/trade.h"
static const u8 sTradeNextSelectedMonTable[(6 * 2) + 1][4][6] =
{
    {
        {4, 2, 12, 12, 0, 0},
        {2, 4, 12, 12, 0, 0},
...
{
    [0] = sText_CommunicationStandby,
    [1] = sText_TheTradeHasBeenCanceled,
    [2] = sText_OnlyPkmnForBattle,
    [3] = gText_OnlyPkmnForBattle,
    [4] = sText_WaitingForYourFriend,
    [5] = sText_YourFriendWantsToTrade,
    [6] = gText_PkmnCantBeTradedNow,
    [7] = gText_EggCantBeTradedNow,
    [8] = gText_OtherTrainersPkmnCantBeTraded
};
...
static const u8 sTradeGfx_PokeBallSymbol[] = INCBIN_U8("graphics/trade/pokeball_symbol.8bpp");
static const u16 sTradeTilemap_Cable[] = INCBIN_U16("graphics/trade/cable_closeup_map.bin");
static const u16 sTradeTilemap_PokeBallSymbol[] = INCBIN_U16("graphics/trade/pokeball_symbol_map.bin");
...
static const struct InGameTrade sIngameTrades[] =
{
    ...
    [3] =
    {
        ...
        .otGender = 1,
        .sheen = 10,
        .requestedSpecies = 315
    }
};

static const u16 sIngameTradeMail[][9 + 1] =
{
    {
        (0x8 << 9) | 0x6,
        (0x9 << 9) | 0x32,
...
static void InitTradeMenu(void)
{
    ...
    InitBgsFromTemplates(0, sTradeMenuBgTemplates, (size_t)(sizeof(sTradeMenuBgTemplates) / sizeof((sTradeMenuBgTemplates)[0])));
    ...
    if (InitWindows(sTradeMenuWindowTemplates))
    {
        ...
        sTradeMenuData->drawPartyState[0] = 0;
        sTradeMenuData->drawPartyState[1] = 0;
...
static void CB2_CreateTradeMenu(void)
{
    ...
    case 0:
        ...
        InitTradeMenu();
        sMessageBoxAllocBuffer = AllocZeroed((size_t)(sizeof(sMessageBoxTileBuffers) / sizeof((sMessageBoxTileBuffers)[0])) * 256);

        for (i = 0; i < (int)(size_t)(sizeof(sMessageBoxTileBuffers) / sizeof((sMessageBoxTileBuffers)[0])); i++)
        {
    ...
    case 1:
        ...
        for (i = 0; i < 6; i++)
        {
            CreateMon(&gEnemyParty[i], 0, 0, 32, 0, 0, 0, 0);
        }
    ...
    case 7:
        ...
        SetGpuReg(0x0, 0);
        SetGpuReg(0x50, 0);
        sTradeMenuData->partyCounts[0] = gPlayerPartyCount;
        sTradeMenuData->partyCounts[1] = gEnemyPartyCount;

        for (i = 0; i < sTradeMenuData->partyCounts[0]; i++)
        {
            struct Pokemon *mon = &gPlayerParty[i];
            sTradeMenuData->partySpriteIds[0][i] = CreateMonIcon(GetMonData(mon, 65),
                                                         SpriteCB_MonIcon,
        ...
        for (i = 0; i < sTradeMenuData->partyCounts[1]; i++)
        {
            struct Pokemon *mon = &gEnemyParty[i];
            sTradeMenuData->partySpriteIds[1][i] = CreateMonIcon(GetMonData(mon, 65, ((void *)0)),
                                                         SpriteCB_MonIcon,
    ...
    case 8:
        LoadHeldItemIcons();
        DrawHeldItemIconsForTrade(&sTradeMenuData->partyCounts[0], sTradeMenuData->partySpriteIds[0], 0);
        gMain.state++;
        break;
    case 9:
        DrawHeldItemIconsForTrade(&sTradeMenuData->partyCounts[0], sTradeMenuData->partySpriteIds[0], 1);
        gMain.state++;
        break;
    case 10:
        ...
        DrawTextWindowAndBufferTiles(sTradeActionTexts[0], sMessageBoxTileBuffers[6], 0, 0, 2);
        DrawTextWindowAndBuffer6Bytes(sTradeActionTexts[1], sMessageBoxTileBuffers[8], 24);
        gMain.state++;
        sTradeMenuData->timer = 0;
        break;
    case 11:
    ...
    case 13:
        ...
        break;
    case 14:
        SetTradePartyLiveStatuses(0);
        PrintPartyNicknamesForTradeMenu(0);
    ...
    case 15:
        ...
        PrintPartyNicknamesForTradeMenu(1);
        gMain.state++;
    ...
    case 19:
        ...
        break;
    case 20:
        GetTradePartyHPBarLevels(0);
        gMain.state++;
        break;
    case 21:
        GetTradePartyHPBarLevels(1);
...
static void CB2_ReturnToTradeMenu(void)
{
    ...
    case 7:
        ...
        {
            struct Pokemon *mon = &gPlayerParty[i];
            sTradeMenuData->partySpriteIds[0][i] = CreateMonIcon(GetMonData(mon, 65, ((void *)0)),
                                                         SpriteCB_MonIcon,
                                                         (sTradeMonSpriteCoords[i][0] * 8) + 14,
                                                         (sTradeMonSpriteCoords[i][1] * 8) - 12,
                                                         1,
    ...
    case 13:
        ...
        if (sTradeMenuData->cursorPosition < 6)
            sTradeMenuData->cursorPosition = gLastViewedMonIndex;
        else
            sTradeMenuData->cursorPosition = gLastViewedMonIndex + 6;
...
static void LinkTradeFadeOut(void)
{
    if (++sTradeMenuData->timer > 15)
    {
        ...
        sTradeMenuData->tradeMenuFunc = 10;
    }
}
...
static void LinkTradeWaitForFade(void)
{
    if (!gPaletteFade.active)
    {
        gSelectedTradeMonPositions[0] = sTradeMenuData->cursorPosition;
        gSelectedTradeMonPositions[1] = sTradeMenuData->partnerCursorPosition;

        if (gWirelessCommType)
        {
            sTradeMenuData->tradeMenuFunc = 16;
        }
        else
        {
            sub_800ABF4(32);
            sTradeMenuData->tradeMenuFunc = 13;
        }
    }
}
...
static void CB2_TradeMenu(void)
{
    CallTradeMenuFunc();
    DoQueuedActions();
    DrawTradeMenuParty(0);
    DrawTradeMenuParty(1);

    SetGpuReg(0x18, sTradeMenuData->bg2hofs++);
    SetGpuReg(0x1c, sTradeMenuData->bg3hofs--);
...
static void LoadTradeBgGfx(u8 state)
{
    ...
    case 1:
        ...
        DrawTradeMenuPartyInfo(0);
        DrawTradeMenuPartyInfo(1);
        CopyBgTilemapBufferToVram(1);
        break;
    case 2:
        for (i = 0; i < 4; i++)
        {
            SetGpuReg(0x10 + (i * 2), 0);
        }
        ShowBg(0);
        ShowBg(1);
        ShowBg(2);
...
static void SetTradePartyMonsVisible(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (i < sTradeMenuData->partyCounts[0])
        {
            gSprites[sTradeMenuData->partySpriteIds[0][i]].invisible = 0;
            sTradeMenuData->monPresent[i] = 1;
        }
        else
        {
            sTradeMenuData->monPresent[i] = 0;
        }

        if (i < sTradeMenuData->partyCounts[1])
        {
            gSprites[sTradeMenuData->partySpriteIds[1][i]].invisible = 0;
            sTradeMenuData->monPresent[i + 6] = 1;
        }
        else
        {
            sTradeMenuData->monPresent[i + 6] = 0;
        }
    }
...
static bool8 BufferTradeParties(void)
{
    ...
    case 16:
        ...
        break;
    case 17:
        Trade_Memcpy(gBlockSendBuffer, gSaveBlock1Ptr->giftRibbons, (size_t)(sizeof(sTradeMenuData->giftRibbons) / sizeof((sTradeMenuData->giftRibbons)[0])));
        sTradeMenuData->bufferPartyState++;
        break;
    case 19:
        if (id == 0)
    ...
    case 20:
        if (_GetBlockReceivedStatus() == 3)
        {
            Trade_Memcpy(sTradeMenuData->giftRibbons, gBlockRecvBuffer[id ^ 1], (size_t)(sizeof(sTradeMenuData->giftRibbons) / sizeof((sTradeMenuData->giftRibbons)[0])));
            TradeResetReceivedFlags();
            sTradeMenuData->bufferPartyState++;
        }
        break;
...
{
    DrawTextWindowAndBuffer6Bytes(sText_IsThisTradeOkay, (void *)((0x6000000 + 0x10000) + (sTradeMenuData->bottomTextTileStart * 32)), 24);
}
...
static void UpdateLinkTradeFlags(u8 mpId, u8 status)
{
    if (status & 1)
    {
        ...
        {
        case 0xEEAA:
            sTradeMenuData->playerLinkFlagChoseAction = 2;
            break;
        case 0xAABB:
            sTradeMenuData->playerLinkFlagChoseAction = 1;
            break;
        case 0xBBBB:
            sTradeMenuData->playerLinkFlagStatus = 1;
            break;
        case 0xBBCC:
            sTradeMenuData->playerLinkFlagStatus = 2;
            break;
        }
    ...
    if (status & 2)
    {
        ...
        {
        case 0xEEAA:
            sTradeMenuData->partnerLinkFlagChoseAction = 2;
            break;
        case 0xAABB:
            sTradeMenuData->partnerCursorPosition = gBlockRecvBuffer[1][1] + 6;
            sTradeMenuData->partnerLinkFlagChoseAction = 1;
            break;
        case 0xBBBB:
            sTradeMenuData->partnerLinkFlagStatus = 1;
            break;
        case 0xBBCC:
            sTradeMenuData->partnerLinkFlagStatus = 2;
            break;
        }
...
static void ReactToLinkTradeData(u8 mpId, u8 status)
{
    if (status & 1)
    {
        ...
        case 0xEEBB:
            BeginNormalPaletteFade(0xFFFFFFFF, 0, 0, 16, ((0) | ((0) << 5) | ((0) << 10)));
            PrintTradeMessage(4);
            sTradeMenuData->tradeMenuFunc = 11;
            break;
        case 0xEECC:
            PrintTradeMessage(5);
            sTradeMenuData->tradeMenuFunc = 8;
            break;
        case 0xDDDD:
...
static void QueueLinkTradeData(void)
{
    ...
    {
        if (sTradeMenuData->playerLinkFlagChoseAction == 1
            && sTradeMenuData->partnerLinkFlagChoseAction == 1)
        {
            sTradeMenuData->tradeMenuFunc = 6;
        ...
        else if (sTradeMenuData->playerLinkFlagChoseAction == 1
              && sTradeMenuData->partnerLinkFlagChoseAction == 2)
        {
            PrintTradeMessage(1);
        ...
        else if (sTradeMenuData->playerLinkFlagChoseAction == 2
              && sTradeMenuData->partnerLinkFlagChoseAction == 1)
        {
            PrintTradeMessage(5);
    ...
    {
        if (sTradeMenuData->playerLinkFlagStatus == 1
         && sTradeMenuData->partnerLinkFlagStatus == 1)
        {
            ...
            sTradeMenuData->playerLinkFlagStatus = 0;
            sTradeMenuData->partnerLinkFlagStatus = 0;
            sTradeMenuData->tradeMenuFunc = 9;
        }

        if (sTradeMenuData->playerLinkFlagStatus == 2
         || sTradeMenuData->partnerLinkFlagStatus == 2)
        {
            PrintTradeMessage(1);
            sTradeMenuData->linkData[0] = 0xDDEE;
...
static u8 GetNewTradeMenuPosition(u8 oldPosition, u8 direction)
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (sTradeMenuData->monPresent[sTradeNextSelectedMonTable[oldPosition][direction][i]] == 1)
        {
...
{
    PrintTradeMessage(0);
    sTradeMenuData->tradeMenuFunc = 100;
...
static void TradeMenuProcessInput(void)
{
    ...
    if (gMain.newKeys & 0x0001)
    {
        ...
        if (sTradeMenuData->cursorPosition < 6)
        {
            ...
            PrintMenuTable(1, (size_t)(sizeof(sSelectTradeMonActions) / sizeof((sSelectTradeMonActions)[0])), sSelectTradeMonActions);
            InitMenuInUpperLeftCornerPlaySoundWhenAPressed(1, 2, 0);
            PutWindowTilemap(1);
            CopyWindowToVram(1, 3);
            sTradeMenuData->tradeMenuFunc = 1;
        }

        else if (sTradeMenuData->cursorPosition < 6 * 2)
        {
            BeginNormalPaletteFade(0xFFFFFFFF, 0, 0, 16, ((0) | ((0) << 5) | ((0) << 10)));
            sTradeMenuData->tradeMenuFunc = 2;
        }

        else if (sTradeMenuData->cursorPosition == 6 * 2)
        {
            CreateYesNoMenu(&sTradeYesNoWindowTemplate, 1, 14, 0);
            sTradeMenuData->tradeMenuFunc = 4;
            DrawTextWindowAndBuffer6Bytes(sTradeActionTexts[4], (void *)((0x6000000 + 0x10000) + sTradeMenuData->bottomTextTileStart * 32), 24);
        }
    }
}
...
static void TradeMenuChooseMon(void)
{
    ...
    sTradeMenuData->tradeMenuFunc = 0;
    gSprites[sTradeMenuData->cursorSpriteIdx].invisible = 0;
    DrawTextWindowAndBuffer6Bytes(sTradeActionTexts[1], (void *)((0x6000000 + 0x10000) + sTradeMenuData->bottomTextTileStart * 32), 24);
}
...
static void TradeMenuProcessInput_SelectedMon(void)
{
    ...
    case 1:
        ...
        case 0:
            SetReadyToTrade();
            gSprites[sTradeMenuData->cursorSpriteIdx].invisible = 1;
            break;
        case 1:
            QueueAction(3, 3);
            sTradeMenuData->tradeMenuFunc = 8;
...
static void TradeMenuShowMonSummaryScreen(void)
{
    if (!gPaletteFade.active)
    {
        ...
        if (sTradeMenuData->cursorPosition < 6)
            ShowPokemonSummaryScreen(PSS_MODE_LOCK_MOVES, gPlayerParty, sTradeMenuData->cursorPosition, sTradeMenuData->partyCounts[0] - 1, CB2_ReturnToTradeMenu);

        else
            ShowPokemonSummaryScreen(PSS_MODE_LOCK_MOVES, gEnemyParty, sTradeMenuData->cursorPosition - 6, sTradeMenuData->partyCounts[1] - 1, CB2_ReturnToTradeMenu);
        FreeAllWindowBuffers();
    }
}
...
static u8 CheckValidityOfTradeMons(u8 *aliveMons, u8 playerPartyCount, u8 playerMonIdx, u8 partnerMonIdx)
{
    ...
    partnerMonIdx %= 6;
    partnerSpecies = GetMonData(&gEnemyParty[partnerMonIdx], 11);


    if (partnerSpecies == 410 || partnerSpecies == 151)
    {
        if (!GetMonData(&gEnemyParty[partnerMonIdx], 80))
            return 2;
    }
    ...
    {
        if (sTradeMenuData->isEgg[1][partnerMonIdx] || !IsSpeciesInHoennDex(partnerSpecies))
            return 2;
    }
...
static bool32 CheckMonsBeforeTrade(void)
{
    ...
    for (i = 0; i < sTradeMenuData->partyCounts[0]; i++)
    {
        aliveMons[i] = sTradeMenuData->isLiveMon[0][i];
    }

    switch (CheckValidityOfTradeMons(aliveMons, sTradeMenuData->partyCounts[0], sTradeMenuData->cursorPosition, sTradeMenuData->partnerCursorPosition))
    {
    case 0:
        QueueAction(3, 3);
...
static void ConfirmOrCancelTrade(void)
{
    ...
    case 0:
        if (!CheckMonsBeforeTrade())
            ...
            sTradeMenuData->tradeMenuFunc = 17;
...
{
    if (sTradeMenuData->drawPartyState[0] == 5
        && sTradeMenuData->drawPartyState[1] == 5)
    {
        PrintAndBufferIsThisTradeOkay();
        sTradeMenuData->tradeMenuFunc = 14;
    }
}
...
static void DelayTradeConfirmation(void)
{
    ...
    if (sTradeMenuData->timer > 120)
    {
        CreateYesNoMenu(&sTradeYesNoWindowTemplate, 1, 14, 0);
        sTradeMenuData->timer = 0;
        sTradeMenuData->tradeMenuFunc = 3;
    }
}
...
static void RedrawTradeMenuAfterPressA(void)
{
    ...
    if (gMain.newKeys & 0x0001)
    {
        ...
        RedrawTradeMenuParty(0);
        RedrawTradeMenuParty(1);
        sTradeMenuData->tradeMenuFunc = 0;
        gSprites[sTradeMenuData->cursorSpriteIdx].invisible = 0;
    }
}
...
static void CancelTrade_1(void)
{
    if (!gPaletteFade.active)
    {
        ...
        sTradeMenuData->tradeMenuFunc = 12;
    }
}
...
{
    if (!sub_801048C(0) && GetNumQueuedActions() == 0)
    {
        sub_800ADF8();
        sTradeMenuData->tradeMenuFunc = 13;
    }
...
static void DrawTradeMenuParty(u8 whichParty)
{
    ...
    selectedMonParty = 1;
    if (sTradeMenuData->selectedMonIdx[whichParty] < 6)
        selectedMonParty = 0;
    ...
    case 1:
        ...
        {
            gSprites[sTradeMenuData->partySpriteIds[0][i + (selectedMonParty * 6)]].invisible = 1;
        }
        ...
        gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]].invisible = 0;
        gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]].data[0] = 20;
        gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]].data[2] = (sTradeMonSpriteCoords[selectedMonParty * 6][0] + sTradeMonSpriteCoords[selectedMonParty * 6 + 1][0]) / 2 * 8 + 14;
        gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]].data[4] = (sTradeMonSpriteCoords[selectedMonParty * 6][1] * 8) - 12;
        StoreSpriteCallbackInData6(&gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]], SpriteCB_MonIcon);
        sTradeMenuData->drawPartyState[whichParty]++;
        TradeMenuBouncePartySprites(&gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]]);
        ...
        break;
    case 2:
        if (gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]].callback == SpriteCB_MonIcon)
    ...
    case 3:
        ...
        CopyBgTilemapBufferToVram(1);
        gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]].pos1.x = (sTradeMonSpriteCoords[selectedMonParty * 6][0] + sTradeMonSpriteCoords[selectedMonParty * 6 + 1][0]) / 2 * 8 + 14;
        gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]].pos1.y = (sTradeMonSpriteCoords[selectedMonParty * 6][1] * 8) - 12;
        gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]].pos2.x = 0;
        gSprites[sTradeMenuData->partySpriteIds[0][partyIdx + (selectedMonParty * 6)]].pos2.y = 0;
...
static void ResetTradeMenuPartyPositions(u8 whichParty)
{
    ...
    for (i = 0; i < sTradeMenuData->partyCounts[whichParty]; i++)
    {
        gSprites[sTradeMenuData->partySpriteIds[whichParty][i]].invisible = 0;
        gSprites[sTradeMenuData->partySpriteIds[whichParty][i]].pos1.x = sTradeMonSpriteCoords[(whichParty * 6) + i][0] * 8 + 14;
        gSprites[sTradeMenuData->partySpriteIds[whichParty][i]].pos1.y = sTradeMonSpriteCoords[(whichParty * 6) + i][1] * 8 - 12;
...
static void QueueAction(u16 delay, u8 actionId)
{
    ...
    for (i = 0; i < (int)(size_t)(sizeof(sTradeMenuData->queuedActions) / sizeof((sTradeMenuData->queuedActions)[0])); i++)
    {
        if (!sTradeMenuData->queuedActions[i].queued)
        {
            ...
            sTradeMenuData->queuedActions[i].queued = 1;
            break;
        }
    }
}
    ...
    for (i = 0; i < (int)(size_t)(sizeof(sTradeMenuData->queuedActions) / sizeof((sTradeMenuData->queuedActions)[0])); i++)
    {
        if (sTradeMenuData->queuedActions[i].queued)
        {
            ...
            else
            {
                ...
                sTradeMenuData->queuedActions[i].queued = 0;
            }
        }
    }
}
...
static void PrintTradeMessage(u8 messageId)
{
    ...
    AddTextPrinterParameterized(0, 1, sTradeMessages[messageId], 0, 1, 0xFF, ((void *)0));
...
static bool8 LoadTradeMenuSpriteSheetsAndPalettes(void)
{
    ...
    if (sTradeMenuData->timer < (int)(size_t)(sizeof(sMessageBoxTileBuffers) / sizeof((sMessageBoxTileBuffers)[0])))
    {
...
static void SetTradePartyLiveStatuses(u8 whichParty)
{
    ...
    case 0:
        for (i = 0; i < sTradeMenuData->partyCounts[whichParty]; i++)
        {
            ...
            {
                sTradeMenuData->isLiveMon[whichParty][i] = 0;
                sTradeMenuData->isEgg[whichParty][i] = 1;
            }
            else if (GetMonData(&gPlayerParty[i], 57) == 0)
            {
                sTradeMenuData->isLiveMon[whichParty][i] = 0;
                sTradeMenuData->isEgg[whichParty][i] = 0;
            }
            else
            {
                sTradeMenuData->isLiveMon[whichParty][i] = 1;
                sTradeMenuData->isEgg[whichParty][i] = 0;
            }
        }
        break;
...
static void GetTradePartyHPBarLevels(u8 who)
{
    ...
    case 0:
        for (i = 0; i < sTradeMenuData->partyCounts[0]; i++)
        {
            ...
            sTradeMenuData->hpBarLevels[0][i] = GetHPBarLevel(curHp, maxHp);
        }
        break;
    case 1:
        for (i = 0; i < sTradeMenuData->partyCounts[1]; i++)
        {
            ...
            sTradeMenuData->hpBarLevels[1][i] = GetHPBarLevel(curHp, maxHp);
        }
        break;
    }
}
...
static void SaveTradeGiftRibbons(void)
{
    ...
    for (i = 0; i < (int)(size_t)(sizeof(sTradeMenuData->giftRibbons) / sizeof((sTradeMenuData->giftRibbons)[0])); i++)
    {
...
static u32 CanTradeSelectedMon(struct Pokemon *playerParty, int partyCount, int monIdx)
{
    ...
    if ((player->version & 0xFF) != 2 &&
        (player->version & 0xFF) != 1)
    {
...
int CanSpinTradeMon(struct Pokemon *mon, u16 monIdx)
{
    ...
    {
        speciesArray[i] = GetMonData(&mon[i], 65);
...
static void SetTradeBGAffine(void)
{
    ...
    SetGpuReg(0x2a, affine.dx >> 16);
    SetGpuReg(0x2c, affine.dy);
    SetGpuReg(0x2e, affine.dy >> 16);
}
...
static void SetTradeGpuRegs(void)
{
    ...
    SetGpuReg(0x16, sTradeData->bg1vofs);
    SetGpuReg(0x14, sTradeData->bg1hofs);

    dispcnt = GetGpuReg(0x0);
    if ((dispcnt & 7) == 0x0000)
    {
        SetGpuReg(0x1a, sTradeData->bg2vofs);
        SetGpuReg(0x18, sTradeData->bg2hofs);
    }
    else
    {
        SetTradeBGAffine();
...
static void CheckForLinkTimeout(void)
{
    ...
    if (sTradeData->linkTimeoutCounter > 300)
    {
        CloseLink();
...
static void LoadTradeMonPic(u8 whichParty, u8 state)
{
    ...
    case 1:
        ...
        gSprites[sTradeData->pokePicSpriteIdxs[whichParty]].invisible = 1;
...
static void InitTradeBgInternal(void)
{
    SetGpuReg(0x0, 0);
    ResetBgsAndClearDma3BusyFlags(0);
    InitBgsFromTemplates(0, sTradeSequenceBgTemplates, (size_t)(sizeof(sTradeSequenceBgTemplates) / sizeof((sTradeSequenceBgTemplates)[0])));
    ChangeBgX(0, 0, 0);
    ChangeBgY(0, 0, 0);
...
static void CB2_InGameTrade(void)
{
    ...
    case 0:
        ...
        gLinkPlayers[1].language = GetMonData(&gEnemyParty[0], 3);
        sTradeData = AllocZeroed(sizeof(*sTradeData));
        AllocateMonSpritesGfx();
        ResetTasks();
        ResetSpriteData();
...
static void SetTradeSequenceBgGpuRegs(u8 state)
{
    ...
    case 2:
        ...
        if (!sTradeData->isCableTrade)
        {
            SetGpuReg(0x0, 0x0001 |
                                          ...
                                          0x0200 |
                                          0x1000);
            LZ77UnCompVram(gUnknown_083379A0, (void *) (0x6000000 + (0x800 * (5))));
            BlendPalettes(0x8, 16, ((0) | ((0) << 5) | ((0) << 10)));
        }
        else
        {
            SetGpuReg(0x0, 0x0001 |
    ...
    case 3:
        ...
        LZ77UnCompVram(sTradeGfx_WirelessSignal, (void *) (0x6000000 + (0x4000 * (1))));
        LZ77UnCompVram(sTradeTilemap_WirelessSignal, (void *) (0x6000000 + (0x800 * (18))));
...
static void BufferTradeSceneStrings(void)
{
    ...
    if (sTradeData->isLinkTrade)
    {
        ...
        GetMonData(&gEnemyParty[gSelectedTradeMonPositions[1] % 6], 2, name);
        StringCopy10(gStringVar3, name);
        GetMonData(&gPlayerParty[gSelectedTradeMonPositions[0]], 2, name);
    ...
    else
    {
        ...
        GetMonData(&gPlayerParty[gSpecialVar_0x8005], 2, name);
        StringCopy10(gStringVar2, name);
    }
}
...
static bool8 AnimateTradeSequenceCable(void)
{
    ...
    {
    case 0:
        gSprites[sTradeData->pokePicSpriteIdxs[0]].invisible = 0;
        gSprites[sTradeData->pokePicSpriteIdxs[0]].pos2.x = -180;
        gSprites[sTradeData->pokePicSpriteIdxs[0]].pos2.y = gMonFrontPicCoords[sTradeData->monSpecies[0]].y_offset;
        sTradeData->state++;
    ...
    case 10:
        ...
        if (sTradeData->monSpecies[0] != 412)
        {
            PlayCry1(sTradeData->monSpecies[0], 0);
        }
    ...
    case 33:
        ...
        break;
    case 34:
        BlendPalettes(0x1, 16, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
        sTradeData->state++;
        break;
    case 35:
        BlendPalettes(0x1, 0, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
        sTradeData->state++;
        break;
    case 36:
        BlendPalettes(0x1, 16, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
        ...
        break;
    case 37:
        if (!IsMonSpriteNotFlipped(sTradeData->monSpecies[0]))
        {
            ...
            CalcCenterToCornerVec(&gSprites[sTradeData->pokePicSpriteIdxs[0]], (((3 << 2) | (0)) & 0x03), ((((3 << 2) | (0)) >> 2) & 0x03), 3);
        ...
        gSprites[sTradeData->pokePicSpriteIdxs[0]].invisible = 0;
        gSprites[sTradeData->pokePicSpriteIdxs[1]].invisible = 0;
        sTradeData->state++;
        break;
    case 38:
        ...
        if (gSprites[sTradeData->pokePicSpriteIdxs[0]].pos2.y < -222)
        {
            ...
            sTradeData->state++;
            gSprites[sTradeData->pokePicSpriteIdxs[0]].invisible = 1;
            gSprites[sTradeData->pokePicSpriteIdxs[1]].invisible = 1;
            BlendPalettes(0x1, 0, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
        }
        break;
    case 39:
    ...
    case 63:
        ...
        BlendPalettes(1 << (16 + gSprites[sTradeData->unk_D3].oam.paletteNum), 16, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
        sTradeData->state++;
        sTradeData->timer = 0;
        break;
    case 64:
        BeginNormalPaletteFade(1 << (16 + gSprites[sTradeData->unk_D3].oam.paletteNum), 1, 16, 0, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
        sTradeData->state++;
        break;
    case 65:
        if (gSprites[sTradeData->unk_D3].callback == SpriteCallbackDummy)
        {
            HandleLoadSpecialPokePic_2(&gMonFrontPicTable[sTradeData->monSpecies[1]], gMonSpritesGfxPtr->sprites[3], sTradeData->monSpecies[1], sTradeData->monPersonalities[1]);
            sTradeData->state++;
        }
        break;
    case 66:
        gSprites[sTradeData->pokePicSpriteIdxs[1]].pos1.x = 120;
        gSprites[sTradeData->pokePicSpriteIdxs[1]].pos1.y = gMonFrontPicCoords[sTradeData->monSpecies[1]].y_offset + 60;
        ...
        CreatePokeballSpriteToReleaseMon(sTradeData->pokePicSpriteIdxs[1], gSprites[sTradeData->pokePicSpriteIdxs[1]].oam.paletteNum, 120, 84, 2, 1, 20, 0xFFFFF, sTradeData->monSpecies[1]);
    ...
    case 74:
        if (!gPaletteFade.active)
        {
            ...
            if (sTradeData)
            {
                ...
                FreeMonSpritesGfx();
                { Free(sTradeData); sTradeData = ((void *)0); };
            }
...
static bool8 AnimateTradeSequenceWireless(void)
{
    ...
    case 33:
        ...
        break;
    case 34:
        BlendPalettes(0x8, 16, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
        sTradeData->state++;
        break;
    case 35:
        BlendPalettes(0x8, 16, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
...
static void UpdateTradeFinishFlags(void)
{
    ...
    if (blockReceivedStatus & 0x01)
    {
        ...
        if (gBlockRecvBuffer[0][0] == 0xABCD)
            sTradeData->playerLinkFlagFinishTrade = 1;
    ...
    if (blockReceivedStatus & 0x02)
    {
        if (gBlockRecvBuffer[1][0] == 0xABCD)
            sTradeData->partnerLinkFlagFinishTrade = 1;
...
static void sub_807E5D8(struct Sprite *sprite)
{
    ...
    if (++ sprite->data[0] == 44)
    {
        ...
        sprite->data[0] = 0;
        BeginNormalPaletteFade(1 << (16 + sprite->oam.paletteNum), -1, 0, 16, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
    }
}
...
static void _CreateInGameTradePokemon(u8 whichPlayerMon, u8 whichInGameTrade)
{
    ...
    SetMonData(pokemon, 2, inGameTrade->nickname);
    SetMonData(pokemon, 7, inGameTrade->otName);
    SetMonData(pokemon, 49, &inGameTrade->otGender);
    SetMonData(pokemon, 46, &inGameTrade->abilityNum);
    ...
    SetMonData(pokemon, 33, &inGameTrade->conditions[3]);
    SetMonData(pokemon, 47, &inGameTrade->conditions[4]);
...
static void CB2_TryFinishTrade(void)
{
    ...
    else
    {
            ...
            && sTradeData->partnerLinkFlagFinishTrade == 1)
        {
            ...
            (SendBlock(bitmask_all_link_players_but_self(), sTradeData->linkData, 20));
            sTradeData->playerLinkFlagFinishTrade = 2;
            sTradeData->partnerLinkFlagFinishTrade = 2;
        }
    }
    RunTasks();
    AnimateSprites();
...
void DrawTextOnTradeWindow(u8 windowId, const u8 *str, u8 speed)
{
    ...
    sTradeData->textColors[0] = 0xF;
    sTradeData->textColors[1] = 0x1;
    sTradeData->textColors[2] = 0x6;
...
static void Task_AnimateWirelessSignal(u8 taskId)
{
    ...
    u16 paletteIdx = sWirelessSignalTiming[data[0]][0] * 16;
    ...
    if (sWirelessSignalTiming[data[0]][0] == 0 && data[1] == 0)
        PlaySE(195);

    if (data[1] == sWirelessSignalTiming[data[0]][1])
    {
        data[0]++;
        data[1] = 0;
        if (sWirelessSignalTiming[data[0]][1] == 0xFF)
        {
            DestroyTask(taskId);
        }
    }
...
static void c3_0805465C(u8 taskId)
{
    ...
    SetGpuReg(0x40, ((sTradeData->wirelessWinRight) | ((sTradeData->wirelessWinLeft) << 8)));
    SetGpuReg(0x44, ((sTradeData->wirelessWinBottom) | ((sTradeData->wirelessWinTop) << 8)));
...
static void sub_807F39C(u8 taskId)
{
    ...
    if (sTradeData->wirelessWinLeft != 120)
    {
        ...
        if (sTradeData->wirelessWinLeft >= 116)
            BlendPalettes(0x8, 0, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
    }
    else
    {
...
static void CB2_SaveAndEndWirelessTrade(void)
{
    ...
    case 10:
        ...
        break;
    case 11:
        if (!gPaletteFade.active && IsBGMStopped() == 1)
        {
            sub_8077288(3);
            gMain.state = 12;
        }
