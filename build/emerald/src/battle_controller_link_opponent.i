...
static void LinkOpponentBufferRunCommand(void)
{
    ...
    {
        if (gBattleBufferA[gActiveBattler][0] < (size_t)(sizeof(sLinkOpponentBufferCommands) / sizeof((sLinkOpponentBufferCommands)[0])))
    ...
    if (r8)
    {
        if (r4 || !IsBattlerSpriteVisible(((gActiveBattler) ^ 2)))
        {
            if (!gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].field_1_x1)
                return;
            if (!gBattleSpritesDataPtr->healthBoxesData[((gActiveBattler) ^ 2)].field_1_x1)
                return;
...
static void sub_8064734(void)
{
    ...
    if (!(gBattleTypeFlags & 0x0040)
        ...
        && !gBattleSpritesDataPtr->healthBoxesData[((gActiveBattler) ^ 2)].flag_x80
    ...
    if (r10)
    {
        ...
        {
            if (gBattleTypeFlags & 0x0040 && GetBattlerPosition(gActiveBattler) == 3)
            {
...
static void LinkOpponentHandleCmd55(void)
{
    ...
    if (gBattleBufferA[gActiveBattler][1] == 0x3)
        gBattleOutcome = gBattleBufferA[gActiveBattler][1];
    else
        gBattleOutcome = gBattleBufferA[gActiveBattler][1] ^ 0x3;
