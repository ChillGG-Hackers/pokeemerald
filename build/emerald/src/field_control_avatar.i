...
{
    if (FlagGet((((0x500 + 864 - 1) + 1) + 0xB)) == 1 && PartyHasMonWithSurf() == 1 && IsPlayerFacingSurfableFishableWater() == 1)
        return EventScript_UseSurf;

    if (MetatileBehavior_IsWaterfall(metatileBehavior) == 1)
    {
        if (FlagGet((((0x500 + 864 - 1) + 1) + 0xE)) == 1 && IsPlayerSurfingNorth() == 1)
...
static bool8 TryStartStepCountScript(u16 metatileBehavior)
{
    ...
    if (!(gPlayerAvatar.flags & (1 << PLAYER_AVATAR_STATE_FISHING)) && !MetatileBehavior_IsForcedMovementTile(metatileBehavior))
    {
