...
static bool8 SkipCopyingMetatileFromSavedMap(u16* mapMetatilePtr, u16 mapWidth, u8 yMode)
{
    ...
    if (sub_80FADE4(*mapMetatilePtr & 0x03FF, yMode) == 1)
        return 1;
    return 0;
}
...
void apply_map_tileset_palette(struct Tileset const *tileset, u16 destOffset, u16 size)
{
    ...
    if (tileset)
    {
        ...
        {
            LoadPalette(((u16*)tileset->palettes) + (6 * 16), destOffset, size);
...
{
    copy_tileset_patterns_to_vram(mapLayout->primaryTileset, 512, 0);
}

void copy_map_tileset2_to_vram(struct MapLayout const *mapLayout)
{
    copy_tileset_patterns_to_vram(mapLayout->secondaryTileset, 1024 - 512, 512);
}

void copy_map_tileset2_to_vram_2(struct MapLayout const *mapLayout)
{
    copy_tileset_patterns_to_vram2(mapLayout->secondaryTileset, 1024 - 512, 512);
}

void apply_map_tileset1_palette(struct MapLayout const *mapLayout)
{
    apply_map_tileset_palette(mapLayout->primaryTileset, 0, 6 * 16 * 2);
}

void apply_map_tileset2_palette(struct MapLayout const *mapLayout)
{
    apply_map_tileset_palette(mapLayout->secondaryTileset, 6 * 16, (13 - 6) * 16 * 2);
}
...
void copy_map_tileset1_tileset2_to_vram(struct MapLayout const *mapLayout)
{
    if (mapLayout)
    {
        copy_tileset_patterns_to_vram2(mapLayout->primaryTileset, 512, 0);
        copy_tileset_patterns_to_vram2(mapLayout->secondaryTileset, 1024 - 512, 512);
    }
}
