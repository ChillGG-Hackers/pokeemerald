...
# 2 "src/contest_link_80FC4F4.c" 2
...
# 3 "src/contest_link_80FC4F4.c" 2
...
# 4 "src/contest_link_80FC4F4.c" 2
...
# 5 "src/contest_link_80FC4F4.c" 2
# 1 "include/link.h" 1
# 98 "include/link.h"
struct LinkStatus
{
...
# 6 "src/contest_link_80FC4F4.c" 2
# 1 "include/pokemon.h" 1
# 7 "src/contest_link_80FC4F4.c" 2
...
# 8 "src/contest_link_80FC4F4.c" 2
# 1 "include/task.h" 1
# 10 "include/task.h"
...
# 9 "src/contest_link_80FC4F4.c" 2
# 1 "include/constants/flags.h" 1
# 10 "src/contest_link_80FC4F4.c" 2
...
void sub_80FC804(u8 taskId)
{
    ...
    case 0:
        if (GetMultiplayerId() == 0)
        {
            if (IsLinkTaskFinished() && sub_80FC4F4(&gRngValue, sizeof(gRngValue)) == 1)
                gTasks[taskId].data[0]++;
        }
        else
        {
...
void sub_80FC998(u8 taskId)
{
    ...
    case 0:
        if (IsLinkTaskFinished())
        {
            if (sub_80FC4F4(&gContestPlayerMonIndex, sizeof(gContestPlayerMonIndex)) == 1)
                gTasks[taskId].data[0]++;
        }
        break;
    case 1:
...
void sub_80FC9F8(u8 taskId)
{
    ...
    case 0:
        if (IsLinkTaskFinished())
        {
            if (sub_80FC4F4(&(gContestResources->status)[gContestPlayerMonIndex].currMove, sizeof((gContestResources->status)[gContestPlayerMonIndex].currMove)) == 1)
                gTasks[taskId].data[0]++;
        }
        break;
    case 1:
...
void sub_80FCC88(u8 taskId)
{
    ...
    case 0:
        if (IsLinkTaskFinished())
        {
            if (sub_80FC4F4((gContestResources->status), 4 * sizeof(struct ContestantStatus)) == 1)
                gTasks[taskId].data[0]++;
        }
        break;
    case 1:
        if (sub_80FC55C())
        {
            memcpy((gContestResources->status), gBlockRecvBuffer[gUnknown_02039F2B], 4 * sizeof(struct ContestantStatus));
            gTasks[taskId].data[0]++;
        }
        break;
    case 2:
