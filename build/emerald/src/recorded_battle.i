...
static bool32 IsRecordedBattleSaveValid(struct RecordedBattleSave *save)
{
    if (save->battleFlags == 0)
        return 0;
    if (save->battleFlags & ((0x0002 | 0x0080 | 0x0010 | 0x0200 | 0x0400 | 0x0800 | 0x1000 | 0x2000 | 0x4000 | 0x1000000 | 0x4000000 | 0x8000000 | 0x10000000 | 0x20000000 | 0x40000000)))
        return 0;
...
bool32 MoveRecordedBattleToSaveData(void)
{
    ...
    if (sBattleFlags & 0x0002)
    {
        battleSave->battleFlags = (sBattleFlags & ~(0x0002 | 0x0020)) | 0x2000000;

        if (sBattleFlags & 0x0004)
        {
            battleSave->battleFlags |= 0x80000000;
        }
        else if (sBattleFlags & 0x0040)
        {
...
static void SetVariablesForRecordedBattle(struct RecordedBattleSave *src)
{
    ...
    gBattleTypeFlags = src->battleFlags | 0x1000000;
        ...
        if (GetBattlerSide(battlerId) != 1)
        {
            ...
            else
            {
                if (sBattleRecords[battlerId][sRecordedBytesNo[battlerId]] == 6)
                {
                    ...
                    if (!(gBattleMons[battlerId].status2 & 0x00200000))
                    {
                        for (j = 0; j < 4; j++)
                        {
